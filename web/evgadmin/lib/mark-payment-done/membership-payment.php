...
foreach ($txnItems as $item) {
    if ($item->type == 'membership') {
        ...
        if ($pald->memberId) {
            $giftedBy = $txn->member != $pald->memberId ? $txn->member : null;
            ...
            if ($ins) {
                if (!$giftedBy) {
                    ...
                    foreach ($otherMemnberships as $itm) {
                        if ($itm->stripeSubscribeId) {
                            $subscriptionStatus = loadModule('stripe')->cancelSubscription($itm->stripeSubscribeId);
                        }
                    }
                    $pixdb->update(
                        'memberships',
                ...
                if (
                    ...
                    if ($subscrpCancelDate && $billingInterval) {
                        ...
                        if ($subscription && isset($subscription->id)) {
                            $pixdb->update('memberships', ['id' => $ins], ['stripeSubscribeId' => $subscription->id]);
                        }
                    }
                }
